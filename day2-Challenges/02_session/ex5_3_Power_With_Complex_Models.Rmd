...
library(simr) # for simulated power calculation
set.seed(42)
```
...
n_obs <- 10 # Observations per group 
group <- rep(1:n_groups, each = n_obs) 
treatment <- rep(c(0, 1), length.out = n_groups * n_obs) 
success <- rbinom(n_groups * n_obs, size = 1, prob = 0.5 + 0.2 * treatment) 

# Create a data frame 
data <- data.frame(group = factor(group), treatment = treatment, success = success) 

# Fit a GLMM 
model <- glmer(success ~ treatment + (1 | group), data = data, family = binomial) 
...
```{r}
# Extend the model to include power analysis 
model_power <- extend(model, along = "group", n = 30) # Increase the number of groups 

# Perform power analysis 
power <- powerSim(model_power, seed = 42, nsim = 100) # Run 100 simulations 
power
```
Questions:
...
(hint: you can use the model_power code above, or extend the model with your own code)

```{r}
# Define a range of group sizes 
group_sizes <- seq(___, __, by = 5) 

# Calculate power for each group size 
power_curve <- powerCurve(___, along = "group", breaks = group_sizes, seed = 42, nsim = 50) 
...
fixef(model)["treatment"] <- 0.4 # Change the treatment effect size 
# Perform power analysis 
...
Another way of increasing the effect size is to increase the number of observations per condition. What impact does this have on power?
...
Power Analysis and Effect Sizes: Learn more about conducting power analyses and how to increase effect sizes here.

...
Statistical power is a critical concept in research design. It tells us how likely we are to detect an effect if it truly exists. In this worksheet, we’ll explore what power is, why it matters, and how to calculate it using R. By the end of this exercise, you’ll understand how power relates to sample size, effect size, and significance level.
...
# Calculate sample size for a two-sample t-test result <- pwr.t.test(d = 0.5, power = 0.8, sig.level = 0.05, type = "two.sample") result
Questions:
...
# Calculate power for different effect sizes effect_sizes <- c(0.2, 0.5, 0.8) power_results <- sapply(effect_sizes, function(d) { pwr.t.test(n = 30, d = d, sig.level = 0.05, type = "two.sample")$power }) # Display results data.frame(EffectSize = effect_sizes, Power = power_results)
Questions:
